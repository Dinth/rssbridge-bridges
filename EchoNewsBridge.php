...
class EchoNewsBridge extends BridgeAbstract {
    ...
    for ($page = 1; $page <= $maxPages; $page++) {
      ...
      foreach ($nodes as $node) {
        ...
        if ($time && !empty($time->datetime)) {
          $timestamp = strtotime($time->datetime);
        }
  ...
  private function cleanContent($html) {
    ...
    // More aggressive content cleaning including WhatsApp ads
    $cleaningPatterns = [
      ...
      // ENHANCED: Universal WhatsApp advertisement removal - multiple formats
      // Format 1: With HR tags (original format)
      '/<hr>\s*<p[^>]*>\s*We\'re now on\s*&nbsp;\s*WhatsApp![^<]*<\/p>\s*<hr>/is',
      '/<hr[^>]*>\s*<p[^>]*>.*?WhatsApp.*?<\/p>\s*<hr[^>]*>/is',

      // Format 2: Standalone paragraph (current format)
      '/<p[^>]*>\s*We\'re now on WhatsApp![^<]*<\/p>/is',
      '/<p[^>]*>We\'re now on WhatsApp![^<]*?bit\.ly\/[^<]*<\/p>/is',

      // Format 3: Any paragraph containing WhatsApp + bit.ly combination
      '/<p[^>]*>.*?We\'re now on WhatsApp.*?bit\.ly\/[^<]*.*?<\/p>/is',

      // Format 4: Remove any remaining WhatsApp promotional content
      '/We\'re now on\s*(?:&nbsp;)?\s*WhatsApp![^<]*?(?:https?:\/\/)?bit\.ly\/[^\s<>]+[^<]*/is',
    ];
    ...
    // Remove any remaining stray HR tags (advertisement leftovers)
  ...
  private function getDebugInfo($articleDom) {
    ...
    $contentSelectors = [
      '#subscription-content' => 'Main content area',
      '.article-body' => 'Article body',
      '.article-first-paragraph' => 'First paragraph',
      '#subscription-replace-entire-article' => 'Full article',
  ...
  private function findArticleBody($articleDom) {
    $selectors = [
      ...
      '.block-article-content .article-body',
      '.content-body',
      '.article-content',
      'main .content',
      '.post-content'
  ...
  private function extractMainImage($articleDom) {
    ...
    $contentSelectors = [
      '#subscription-content img[srcset]',
      '#subscription-content img[src]',
      '.article-body img[srcset]',
  ...
  private function createExcerpt($bodyDom, $maxLength = 200) {
    ...
    foreach ($paragraphs as $p) {
      ...
      // Skip paragraphs that are unwanted content including WhatsApp ads
      if (strlen($pText) < 20 ||
        ...
        stripos($pText, 'join our new channel') !== false ||
        stripos($pText, 'bit.ly/') !== false ||
        // More specific WhatsApp detection
    ...
    if (strlen(trim($text)) < 50) {
      ...
      if ($firstPara) {
        ...
        // Check if first paragraph is also WhatsApp content
